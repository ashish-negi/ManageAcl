...
<?php
$uglyIdent = Configure::read('ManageAcl.uglyIdent'); 
$lastIdent = null;
foreach ($acos as $id => $aco) {
	...
	//if ($ident != $lastIdent) {
		?><tr class='head-txt manageacl-ident-<?php echo $ident; ?>' data-id="<?php echo $id; ?>" data-parent-id="<?php echo $parent_id; ?>"><?php
	//}
        ?><td><?php if($ident != 2) { ?><a class="show-row" href="javascript:void(0);"><span class="span-plus"></span><?php } echo ($ident == 1 ? "<strong>" : "" ) . ($uglyIdent ? str_repeat("&nbsp;&nbsp;", $ident) : "") . h($alias) . ($ident == 1 ? "</strong>" : "" ); ?><?php if($ident != 2) { ?></a><?php } ?></td>
	<?php foreach ($aros as $aro): 
...
<div class="actions">
	...
	<?php 
	$aroModels = Configure::read("ManageAcl.aros");
	if ($aroModels > 1): ?>
